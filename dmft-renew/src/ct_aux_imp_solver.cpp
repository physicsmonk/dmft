...
NMatrix::NMatrix(std::shared_ptr<const BareGreenFunction> G0, const double gamma, const double K) : _G0(G0), _expg(std::exp(gamma)), _K(K) {
    fermi_sign = 1.0;   // Zero vertex-order expansion is just 1, so its sign is 1.
}
...
std::pair<double, bool> NMatrix::tryInsertVertex(const vertex& v, const double barrier) {
    assert(v.tau >=0 && v.tau < _G0->inverseTemperature() && v.site < _G0->nSites() && (v.aux_spin == 1 || v.aux_spin == -1));
    ...
    for (s = 0; s < 2; ++s) inv_diag[s] = expV(s, v.aux_spin) - _G0->valsOnTauGrid()(s, 0)(v.site, v.site) * (expV(s, v.aux_spin) - 1);  // Always a real number
    ...
    if (vertices.size() > 0) {
        ...
        for (s = 0; s < 2; ++s) {
            inv_col[s].resize(vertices.size());
            inv_row[s].resize(vertices.size());
            for (i = 0; i < vertices.size(); ++i) {
                inv_col[s](i) = -_G0->interpValAtExtendedTau(s, vertices[i].site, v.site, vertices[i].tau - v.tau) * (expV(s, v.aux_spin) - 1);
                inv_row[s](i) = -_G0->interpValAtExtendedTau(s, v.site, vertices[i].site, v.tau - vertices[i].tau) * (expV(s, vertices[i].aux_spin) - 1);
            }
        }
        
        // Calculate acceptance rate
        for (s = 0; s < 2; ++s) {
            colt[s].resize(vertices.size());
            colt[s].noalias() = N[s] * inv_col[s];
            diag[s] = 1.0 / (inv_diag[s] - (inv_row[s] * colt[s])(0, 0));
        }
        acceptance.first = std::real((_K / (vertices.size() + 1.0)) / (diag[0] * diag[1]));
        // Test
        ...
        if ((barrier < 0 || barrier <= std::fabs(acceptance.first)) && barrier <= 1) {
            ...
            Eigen::RowVectorXcd row(vertices.size());
            
            // Calculate the (diagonal elements) columns, rows, block matrices to construct the new matrices
            for (s = 0; s < 2; ++s) {
                // col = -colt1 * diag1;
                row.noalias() = -diag[s] * (inv_row[s] * N[s]);
                N[s].noalias() -= colt[s] * row;  // mat1 = mat1 - colt1 * row
                // Enlarge the matrices with the original matrices at the topleft corner untouched
                N[s].conservativeResize(vertices.size() + 1, vertices.size() + 1);
                // Perform insertion
                N[s].topRightCorner(vertices.size(), 1) = -colt[s] * diag[s];
                N[s].bottomLeftCorner(1, vertices.size()) = row;
                N[s](vertices.size(), vertices.size()) = diag[s];
            }
            ...
            fermi_sign *= copysign(1.0, acceptance.first);
        ...
        acceptance.first = std::real(_K / (diag[0] * diag[1]));
...
std::pair<double, bool> NMatrix::tryRemoveVertex(const std::size_t p, const double barrier) {
    ...
    assert(p < vertices.size());
    
    int s;
    
    acceptance.first = std::real((vertices.size() / _K) * N[0](p, p) * N[1](p, p));
    // Test
        ...
        if (vertices.size() > 1) {
            if (p == 0) {
                for (s = 0; s < 2; ++s) {
                    ...
                    N[s].bottomRightCorner(vertices.size() - 1, vertices.size() - 1).noalias() -= (N[s].bottomLeftCorner(vertices.size() - 1, 1) * N[s].topRightCorner(1, vertices.size() - 1)) / N[s](p, p);
                    N[s] = N[s].bottomRightCorner(vertices.size() - 1, vertices.size() - 1).eval();  // Call eval() to evaluate to temporary
                }
            }
            else if (p < vertices.size() - 1) {
                Eigen::MatrixXcd bmat;
                Eigen::VectorXcd col(vertices.size() - 1);
                Eigen::RowVectorXcd row(vertices.size() - 1);
                
                for (s = 0; s < 2; ++s) {
                    bmat.resize(vertices.size() - 1, vertices.size() - 1);   // Just before this resize bmat was empty because it was moved
                    bmat.topLeftCorner(p, p) = N[s].topLeftCorner(p, p);
                    bmat.topRightCorner(p, vertices.size() - p - 1) = N[s].topRightCorner(p, vertices.size() - p - 1);
                    bmat.bottomLeftCorner(vertices.size() - p - 1, p) = N[s].bottomLeftCorner(vertices.size() - p - 1, p);
                    bmat.bottomRightCorner(vertices.size() - p - 1, vertices.size() - p - 1) = N[s].bottomRightCorner(vertices.size() - p - 1, vertices.size() - p - 1);
                    col.head(p) = N[s].block(0, p, p, 1);
                    col.tail(vertices.size() - p - 1) = N[s].block(p + 1, p, vertices.size() - p - 1, 1);
                    row.head(p) = N[s].block(p, 0, 1, p);
                    row.tail(vertices.size() - p - 1) = N[s].block(p, p + 1, 1, vertices.size() - p - 1);
                    // N1.noalias() = bmat - (col * row) / diag1;
                    bmat.noalias() -= (col * row) / N[s](p, p);
                    N[s] = std::move(bmat);  // Use move assignment because we know bmat is temporary
                }
            }
            else if (p == vertices.size() - 1) {
                for (s = 0; s < 2; ++s) {
                    ...
                    // N1.noalias() -= (col * row) / diag1;
                    N[s].topLeftCorner(vertices.size() - 1, vertices.size() - 1).noalias() -= (N[s].topRightCorner(vertices.size() - 1, 1) * N[s].bottomLeftCorner(1, vertices.size() - 1)) / N[s](p, p);
                    N[s].conservativeResize(vertices.size() - 1, vertices.size() - 1);  // conservativeResize just removes the right-most column and bottom-most row
                }
            }
        }
        else if (vertices.size() == 1) {
            for (s = 0; s < 2; ++s) N[s].resize(0, 0);
        }
...
std::pair<double, bool> NMatrix::tryShiftTau(const std::size_t p, const double tau, const double barrier) {
    ...
    if (vertices.size() > 0) {
        assert(p < vertices.size() && tau >= 0 && tau <= _G0->inverseTemperature());
        
        // Make a copy of the original N matrices and configuration
        std::array<Eigen::MatrixXcd, 2> N0 = N;
        std::vector<vertex> vertices0(vertices);
        double fermi_sign0 = fermi_sign;
        ...
        if (!acceptance.second) {
            ...
            N = std::move(N0);  // Use move assignment because N0 is temporary
            vertices = std::move(vertices0);
            fermi_sign = fermi_sign0;
        }
    }
...
std::pair<double, bool> NMatrix::tryShiftSite(const std::size_t p, const std::size_t site, const double barrier) {
    ...
    if (vertices.size() > 0) {
        assert(p < vertices.size() && site < _G0->nSites());
...
std::pair<double, bool> NMatrix::tryFlipAuxSpin(const std::size_t p, const double barrier) {
    ...
    if (vertices.size() > 0) {
        ...
        for (s = 0; s < 2; ++s) {
            eV[s] = expV(s, vertices[p].aux_spin);
            gam[s] = 1.0 / (eV[s] * eV[s]) - 1.0;
            NG0[s] = (N[s](p, p) * eV[s] - 1.0) / (eV[s] - 1.0);
        ...
        if ((barrier < 0 || barrier <= std::fabs(acceptance.first)) && barrier <= 1) {
            // Flip is accepted
            Eigen::VectorXcd col(vertices.size());
...
//                }
                col = (eV[s] / (eV[s] - 1.0) * lam) * N[s].col(p);
                col(p) = (NG0[s] - 1.0) * lam;
                row = N[s].row(p);  // If not made this tmp, the implicit tmp in the next step will be of the size of N[s]
                N[s].noalias() += col * row;  // mat1 = mat1 + col * row
            }
            
            // Flip the auxiliary spin
            vertices[p].aux_spin *= -1;
            // Update Fermionic sign
            // fermi_sign *= (acceptance.first < 0) ? -1.0 : 1.0;
            fermi_sign *= copysign(1.0, acceptance.first);
...
void NMatrix::reset() {
    vertices.clear();
    for (int s = 0; s < 2; ++s) N[s].resize(0, 0);
    fermi_sign = 1.0;
}
...
void CTAUXImpuritySolver::measAccumGFfCoeffsCorr() {
    assert(_problem->G->fourierCoeffs().dim0() == _problem->G0->fourierCoeffs().dim0());
    if (_problem->G0->tauGridSizeOfExpiwt() < 2) throw std::invalid_argument("Tau grid size of expiwt array is less than 2 so cannot utilize pre-calculated exp(iwt) to measure Gc(iw)!");
    
    const std::size_t nfcut = _problem->G->freqCutoff();
    if (nfcut != _problem->G0->freqCutoff()) throw std::range_error( "Frequency cutoffs of G and G0 do not match!" );
    const std::size_t nc = _problem->G->nSites();
    if (nc != _problem->G0->nSites()) throw std::range_error( "Numbers of sites of G and G0 do not match!" );
    const double beta = _problem->G0->inverseTemperature();
    if (std::fabs(beta - _problem->G->inverseTemperature()) > 1e-9) throw std::range_error( "Temperatures of G and G0 do not match!" );
    
    Eigen::VectorXd eV_1(vertices.size());
    Eigen::MatrixXcd M(vertices.size(), vertices.size()), G0l(nc, vertices.size()), G0r(vertices.size(), nc), MG(vertices.size(), nc);
    int s;
    std::size_t o, p;
    
    Eigen::Array<std::size_t, Eigen::Dynamic, 1> tau_ind4eiwt(vertices.size());
    const double dtau_eiwt = beta / (_problem->G0->tauGridSizeOfExpiwt() - 1);
    
    const int nrandmeasure = 10;
    Eigen::ArrayXXd randtaudiffs(vertices.size(), nrandmeasure), beta_randtaudiffs(vertices.size(), nrandmeasure), sgns(vertices.size(), nrandmeasure);
    int rm;
    std::size_t x;
    ...
    for (p = 0; p < vertices.size(); ++p) tau_ind4eiwt(p) = static_cast<std::size_t>(vertices[p].tau / dtau_eiwt + 0.5);
    
    // Set random tau differences; same for up and down spins
    for (rm = 0; rm < nrandmeasure; ++rm) {
        rdt = beta * urd(reng);
        for (p = 0; p < vertices.size(); ++p) {
            randtaudiffs(p, rm)  = vertices[p].tau - rdt;
...
//        }
        for (p = 0; p < vertices.size(); ++p) eV_1(p) = expV(s, vertices[p].aux_spin) - 1.0;
        M.noalias() = eV_1.asDiagonal() * N[s];
        
        for (o = 0; o <= nfcut; ++o) {
            // Construct the Green's function matrices
            for (p = 0; p < vertices.size(); ++p)
                G0l.col(p) = _problem->G0->fourierCoeffs()(s, o).col(vertices[p].site) * _problem->G0->expiwt(tau_ind4eiwt(p), o);
            for (x = 0; x < nc; ++x) {
                for (p = 0; p < vertices.size(); ++p)
                    G0r(p, x) = _problem->G0->fourierCoeffs()(s, o, vertices[p].site, x) / _problem->G0->expiwt(tau_ind4eiwt(p), o);
            }
            MG.noalias() = M * G0r;
            // gc.noalias() = (G0l * MG) / beta;
            // _problem->G->fourierCoeffs()(s, o) += fermi_sign * gc;
            _problem->G->fourierCoeffs()(s, o).noalias() += (fermi_sign / beta) * (G0l * MG);
        ...
        for (rm = 0; rm < nrandmeasure; ++rm) {
            for (x = 0; x < nc; ++x) {
                for (p = 0; p < vertices.size(); ++p) G0r(p, x) = _problem->G0->interpValAtExtendedTau(s, vertices[p].site, x, randtaudiffs(p, rm));
            }
            MG.noalias() = M * G0r;
            for (x = 0; x < nc; ++x) {
                for (p = 0; p < vertices.size(); ++p) denssample(x) += std::real(sgns(p, rm) * _problem->G0->interpValAtExtendedTau(s, x, vertices[p].site, beta_randtaudiffs(p, rm)) * MG(p, x));
            }
        }
        denssample /= nrandmeasure;
        _problem->G->elecDensities().col(s) += fermi_sign * denssample;
        _problem->G->elecDensVars().col(s) += fermi_sign * denssample.cwiseAbs2();
    }
    
    measured_fermi_sign += fermi_sign;
...
//    }
    ...
    const std::size_t nbins4S = _problem->G->nTauBins4selfEgf();
    const double binsize4S = beta / nbins4S;
    // const double dtau4eiwt = beta / (_problem->G0->tauGridSizeOfExpiwt() - 1);
    Eigen::VectorXd eV_1(vertices.size());
    Eigen::MatrixXcd M(vertices.size(), vertices.size()), G0r(vertices.size(), nc), MG(vertices.size(), nc);
    const int nrandmeasure = 10;
    Eigen::ArrayXXd randtaudiffs(vertices.size(), nrandmeasure), beta_randtaudiffs(vertices.size(), nrandmeasure), sgns(vertices.size(), nrandmeasure);
    Eigen::Array<std::size_t, Eigen::Dynamic, Eigen::Dynamic> ibins4S(vertices.size(), nrandmeasure);
    int rm;
    double rdt;
    std::size_t p, x;
    Eigen::VectorXd denssample(nc);
...
//        }
        ...
        for (rm = 0; rm < nrandmeasure; ++rm) {
            ...
            for (x = 0; x < nc; ++x) {
                for (p = 0; p < vertices.size(); ++p) {
                    _problem->G->selfEgf()(s, ibins4S(p, rm))(vertices[p].site, x) += (fermi_sign * sgns(p, rm) / (binsize4S * nrandmeasure)) * MG(p, x);
                    denssample(x) += std::real(sgns(p, rm) * _problem->G0->interpValAtExtendedTau(s, x, vertices[p].site, beta_randtaudiffs(p, rm)) * MG(p, x));
                }
            }
        }
        denssample /= nrandmeasure;
...
std::pair<int, std::pair<double, bool> > CTAUXImpuritySolver::move1markovStepEqualProp() {
    const std::size_t nc = _problem->G0->nSites();
    const double beta = _problem->G0->inverseTemperature();
    std::pair<int, std::pair<double, bool> > info;
    std::pair<double, bool> acceptance;
    double barrier = urd(reng);
    
    if (vertices.size() == 0) {
        // Try to insert a vertex
        const vertex v = {urd(reng) * beta, static_cast<std::size_t>(urd(reng) * nc), static_cast<int>(urd(reng) * 2) * 2 - 1}; // auxiliary spin is -1 or 1
        
        acceptance = tryInsertVertex(v, barrier);
        info.first = 0;
    }
    else {
        double propose = urd(reng);
        
        if (propose < 1.0 / 3.0) {
            // Try to insert a vertex
            const vertex v = {urd(reng) * beta, static_cast<std::size_t>(urd(reng) * nc), static_cast<int>(urd(reng) * 2) * 2 - 1};
            ...
            const auto p = static_cast<std::size_t>(urd(reng) * vertices.size());
            ...
            if (shifttausite && w2s < cp) {
                // Try shift tau
                acceptance = tryShiftTau(p, urd(reng) * beta, barrier);
                info.first = 2;
            }
            else if (shifttausite && nc > 1 && w2s < 2.0 / 3.0) {
                // Try shift site
                auto site = static_cast<std::size_t>(urd(reng) * (nc - 1));
                if (site >= vertices[p].site) {
                    ++site;
                }
...
CTAUXImpuritySolver::CTAUXImpuritySolver(std::shared_ptr<ImpurityProblem> problem) : _problem(problem), NMatrix(problem->G0, acosh(1 + problem->U * problem->G0->inverseTemperature() * problem->G0->nSites() / (2 * problem->K)), problem->K), urd(0.0, 1.0) {
    ...
    _oldseed = rd() + problem->G->fourierCoeffs().processRank() * 137u;
    ...
    parameters["random seed"] = _oldseed;
...
double CTAUXImpuritySolver::solve() {
    ...
    const std::size_t lsize = neffstep_globtot / _problem->G->fourierCoeffs().processSize();
    const std::size_t neffstep = (_problem->G->fourierCoeffs().processRank() < _problem->G->fourierCoeffs().processSize() - 1) ? lsize : lsize + neffstep_globtot % _problem->G->fourierCoeffs().processSize();
    
    const auto verbosity = std::any_cast<std::string>(parameters.at("verbosity"));
    // Hide cursor
    if (verbosity == "progress_bar" && _problem->G->fourierCoeffs().processRank() == 0) indicators::show_console_cursor(false);
    ...
    indicators::BlockProgressBar bar{
        ...
        indicators::option::PrefixText{"    Markov walking on process " + std::to_string(_problem->G->fourierCoeffs().processRank()) + " "},
    ...
    voHistogram.resize(std::any_cast<std::size_t>(parameters.at("histogram max order")));  // No-op if sizes match
    ...
    for (std::size_t step = 0; step < warmup + neffstep && qmcduration.count() <= qmcdurlim; ++step) {
        ...
        if (verbosity == "on" && _problem->G->fourierCoeffs().processRank() == 0) {
            std::cout << std::setw(15) << nmarkovstep << std::setw(15) << mstepinfo.first << std::setw(15) << mstepinfo.second.first << std::setw(15)
            << mstepinfo.second.second << std::setw(15) << vertices.size() << std::endl;
        }
        
        // Measure
        if (does_measure && step >= warmup && (step - warmup) % measureperiod == 0) {
            // Count average vertex order and vertex order histogram
            avevertorder += vertices.size();
            if (vertices.size() < voHistogram.size()) ++voHistogram[vertices.size()];
        ...
        // Update progress bar
        if (verbosity == "progress_bar" && _problem->G->fourierCoeffs().processRank() == 0) bar.tick();
    }
    // QMC Markov random walk ends here
    
    if (verbosity == "progress_bar" && _problem->G->fourierCoeffs().processRank() == 0) {
    ...
    if (verbosity == "on" && _problem->G->fourierCoeffs().processRank() == 0)
        std::cout << std::setw(22) << "----------------------" << std::setw(16) << " #measurements: " << std::setw(15) << nmeasure << std::setw(22)
        << "----------------------" << std::endl;
    
    // Combine counters on all processes
    MPI_Allreduce(MPI_IN_PLACE, &measured_fermi_sign, 1, MPI_DOUBLE, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
    MPI_Allreduce(MPI_IN_PLACE, &nmeasure, 1, my_MPI_SIZE_T, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
    MPI_Allreduce(MPI_IN_PLACE, &nmarkovstep, 1, my_MPI_SIZE_T, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
    MPI_Allreduce(MPI_IN_PLACE, &avevertorder, 1, MPI_DOUBLE, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
    MPI_Allreduce(MPI_IN_PLACE, voHistogram.data(), static_cast<int>(voHistogram.size()), my_MPI_SIZE_T, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
    measured_fermi_sign /= nmeasure;
    avevertorder /= nmeasure;
    
    if (verbosity == "on" && _problem->G->fourierCoeffs().processRank() == 0)
        std::cout << std::setw(15) << "---------------" << std::setw(29) << " #global total measurements: " << std::setw(15) << nmeasure
        << std::setw(16) << "----------------" << std::endl;
    
    // Finalize measurement
    if (does_measure) {
        // Finalize the measurement of electron density
        MPI_Allreduce(MPI_IN_PLACE, _problem->G->elecDensities().data(), static_cast<int>(_problem->G->elecDensities().size()), MPI_DOUBLE, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
        MPI_Allreduce(MPI_IN_PLACE, _problem->G->elecDensVars().data(), static_cast<int>(_problem->G->elecDensVars().size()), MPI_DOUBLE, MPI_SUM, _problem->G->fourierCoeffs().mpiCommunicator());
        _problem->G->elecDensities() /= nmeasure * measured_fermi_sign;
        _problem->G->elecDensVars() = ((_problem->G->elecDensVars() / (nmeasure * measured_fermi_sign) - _problem->G->elecDensities().cwiseAbs2()) / (nmeasure - 1)).cwiseSqrt();
        for (s = 0; s < 2; ++s) _problem->G->elecDensities().col(s) += _problem->G0->valsOnTauGrid()(s, _problem->G0->tauGridSize() - 1).diagonal().real();
        
        // Use measured electron densities to compute G's high-frequency expansion coefficients
        _problem->G->computeHighFreqExpan(*(_problem->H0), _problem->U);
        
        if (measure_what == "S") {
            // Combine accumulated measurements of S on all processes and process them
            _problem->G->selfEgf().allSum();
            _problem->G->selfEgf()() /= nmeasure * measured_fermi_sign;  // Average value of measured quantity
            ...
            simpinterror = _problem->G->evalFromSelfEgf(*(_problem->G0));
        }
        else if (measure_what == "G") {
            auto Gwmastpart = _problem->G->fourierCoeffs().mastFlatPart();
            ...
            Gwmastpart() = Gwmastpart() / (nmeasure * measured_fermi_sign) + _problem->G0->fourierCoeffs().mastFlatPart()();
            ...
            _problem->G->invFourierTrans();  // Not required for simulation, maybe required in future versions, but just for output for now
        }
    }
...
void CTAUXImpuritySolver::updateInterPhysParams() {
    double gamma = acosh(1 + _problem->U * _problem->G0->inverseTemperature() * _problem->G0->nSites() / (2 * _problem->K));
    // Update inherited parameters
    setParams(gamma, _problem->K);
}
