...
private:
    ...
    //Eigen::Array<double, _n1, 1> m_x;
    std::reference_wrapper<const Eigen::Array<double, _n1, 1> > m_x;   // A const reference to the x data
    ...
    //Eigen::Matrix<_ScalarT, _n1, _n_mat_elems> m_y;
    std::reference_wrapper<const SqMatArray<_ScalarT, _n0, _n1, _nm> > m_y;  // A const reference to the y data
...
public:
    ...
    // No default constructor since members m_x and m_y are references. This simple constructor is just to link the external x and y data
    CubicSplineMat(const Eigen::Array<double, _n1, 1>& x, const SqMatArray<_ScalarT, _n0, _n1, _nm>& y) : m_x(x), m_y(y), m_dx(x(1) - x(0)) {assert(x.size() == y.dim1() && x.size() > 1);}
...
inline std::size_t CubicSplineMat<_ScalarT, _n0, _n1, _nm>::flatIndex(const std::size_t i0, const std::size_t im0, const std::size_t im1) const {
    assert(i0 < m_y.get().dim0() && im0 < m_y.get().dimm() && im1 < m_y.get().dimm());
    return (i0 * m_y.get().dimm() + im1) * m_y.get().dimm() + im0;
}
...
void CubicSplineMat<_ScalarT, _n0, _n1, _nm>::build(const SqMatArray<_ScalarT, _n0, 1, _nm>& G1, const SqMatArray<_ScalarT, _n0, 1, _nm>& G2) {
    ...
    const std::size_t np = m_x.get().size();
    ...
    Eigen::Matrix<_ScalarT, _n1, m_sc_nmatelems> B(np, m_y.get().dim0() * m_y.get().dimm() * m_y.get().dimm());
    ...
    A(0, np - 1) = 1.0;
    for (ix = 1; ix < np - 1; ++ix) {
        A(ix, ix - 1) = (m_x.get()(ix) - m_x.get()(ix - 1)) / (m_x.get()(ix + 1) - m_x.get()(ix - 1));
        A(ix, ix + 1) = 1.0 - A(ix, ix - 1);
    }
    // Last row, for the boundary condition y'(x_min) + y'(x_max) = G1
    A(np - 1, 0) = -2.0;
    A(np - 1, 1) = -1.0;
    A(np - 1, np - 2) = (m_x.get()(np - 1) - m_x.get()(np - 2)) / (m_x.get()(1) - m_x.get()(0));
    A(np - 1, np - 1) = 2.0 * A(np - 1, np - 2);
    
    // Construct the rhs-value matrix
    for (i0 = 0; i0 < m_y.get().dim0(); ++i0) {
        for (im1 = 0; im1 < m_y.get().dimm(); ++im1) {
            for (im0 = 0; im0 < m_y.get().dimm(); ++im0) {
                B(0, flatIndex(i0, im0, im1)) = G2(i0, 0, im0, im1);
                for (ix = 1; ix < np - 1; ++ix) {
                    B(ix, flatIndex(i0, im0, im1)) = 6.0 / (m_x.get()(ix + 1) - m_x.get()(ix - 1)) * ((m_y.get()(i0, ix + 1, im0, im1) - m_y.get()(i0, ix, im0, im1)) / (m_x.get()(ix + 1) - m_x.get()(ix)) - (m_y.get()(i0, ix, im0, im1) - m_y.get()(i0, ix - 1, im0, im1)) / (m_x.get()(ix) - m_x.get()(ix - 1)));
                }
                B(np - 1, flatIndex(i0, im0, im1)) = 6.0 / (m_x.get()(1) - m_x.get()(0)) * (G1(i0, 0, im0, im1) - (m_y.get()(i0, 1, im0, im1) - m_y.get()(i0, 0, im0, im1)) / (m_x.get()(1) - m_x.get()(0)) - (m_y.get()(i0, np - 1, im0, im1) - m_y.get()(i0, np - 2, im0, im1)) / (m_x.get()(np - 1) - m_x.get()(np - 2)));
            }
        }
    }
    ...
    if constexpr (std::is_same<_ScalarT, double>::value) {
        m_deriv2 = dec.solve(B);
    }
    else if (std::is_same<_ScalarT, std::complex<double> >::value) {
        m_deriv2 = dec.solve(B.real()) + 1i * dec.solve(B.imag());
    }
}
...
template <typename _ScalarT, int _n0, int _n1, int _nm>
CubicSplineMat<_ScalarT, _n0, _n1, _nm>::CubicSplineMat(const Eigen::Array<double, _n1, 1>& x, const SqMatArray<_ScalarT, _n0, _n1, _nm>& y, const SqMatArray<_ScalarT, _n0, 1, _nm>& G1, const SqMatArray<_ScalarT, _n0, 1, _nm>& G2) : m_x(x), m_y(y), m_dx(x(1) - x(0)) {
    build(G1, G2);
}
...
_ScalarT CubicSplineMat<_ScalarT, _n0, _n1, _nm>::operator()(const std::size_t i0, const double x0, const std::size_t im0, const std::size_t im1) const {
    assert(x0 >= m_x.get()(0) && x0 <= m_x.get()(m_x.get().size() - 1));
    
    const std::size_t j0 = flatIndex(i0, im0, im1);
    std::size_t x0si = std::lower_bound(m_x.get().cbegin(), m_x.get().cend() - 1, x0) - m_x.get().cbegin();
    if (x0si > 0) --x0si;
    const double Dx0 = x0 - m_x.get()(x0si);
    const double Dx1 = m_x.get()(x0si + 1) - x0;
    const double dx = m_x.get()(x0si + 1) - m_x.get()(x0si);
    const double dxsq = dx * dx;
    
    return (m_deriv2(x0si, j0) * Dx1 * Dx1 * Dx1 / 6.0 + m_deriv2(x0si + 1, j0) * Dx0 * Dx0 * Dx0 / 6.0
            + (m_y.get()(i0, x0si, im0, im1) - m_deriv2(x0si, j0) * dxsq / 6.0) * Dx1
            + (m_y.get()(i0, x0si + 1, im0, im1) - m_deriv2(x0si + 1, j0) * dxsq / 6.0) * Dx0) / dx;
}
...
_ScalarT CubicSplineMat<_ScalarT, _n0, _n1, _nm>::equidistAt(const std::size_t i0, const double x0, const std::size_t im0, const std::size_t im1) const {
    ...
    const std::size_t j0 = flatIndex(i0, im0, im1);
    const std::size_t x0si = std::min(static_cast<std::size_t>((x0 - m_x.get()(0)) / m_dx), static_cast<std::size_t>(m_x.get().size() - 2));
    const double Dx0 = x0 - m_x.get()(x0si);
    const double Dx1 = m_x.get()(x0si + 1) - x0;
    ...
    return (m_deriv2(x0si, j0) * Dx1 * Dx1 * Dx1 / 6.0 + m_deriv2(x0si + 1, j0) * Dx0 * Dx0 * Dx0 / 6.0
            + (m_y.get()(i0, x0si, im0, im1) - m_deriv2(x0si, j0) * dxsq / 6.0) * Dx1
            + (m_y.get()(i0, x0si + 1, im0, im1) - m_deriv2(x0si + 1, j0) * dxsq / 6.0) * Dx0) / m_dx;
}
...
std::complex<double> CubicSplineMat<_ScalarT, _n0, _n1, _nm>::fourierTransform(const std::size_t i0, const double omega, const std::size_t im0, const std::size_t im1) const {
    const std::size_t dim1 = m_x.get().size();
    const std::size_t j0 = flatIndex(i0, im0, im1);
    
    const std::complex<double> eiwtbegin = std::exp(1i * std::fmod(omega * m_x.get()(0), 2 * M_PI));
    const std::complex<double> eiwtend = std::exp(1i * std::fmod(omega * m_x.get()(dim1 - 1), 2 * M_PI));
    
    const _ScalarT deriv1begin = -(m_x.get()(1) - m_x.get()(0)) / 6.0 * (2.0 * m_deriv2(0, j0) + m_deriv2(1, j0))
    + (m_y.get()(i0, 1, im0, im1) - m_y.get()(i0, 0, im0, im1)) / (m_x.get()(1) - m_x.get()(0));
    const _ScalarT deriv1end = (m_x.get()(dim1 - 1) - m_x.get()(dim1 - 2)) / 6.0 * (m_deriv2(dim1 - 2, j0) + 2.0 * m_deriv2(dim1 - 1, j0))
    + (m_y.get()(i0, dim1 - 1, im0, im1) - m_y.get()(i0, dim1 - 2, im0, im1)) / (m_x.get()(dim1 - 1) - m_x.get()(dim1 - 2));
    const _ScalarT deriv3begin = (m_deriv2(1, j0) - m_deriv2(0, j0)) / (m_x.get()(1) - m_x.get()(0));
    const _ScalarT deriv3end = (m_deriv2(dim1 - 1, j0) - m_deriv2(dim1 - 2, j0)) / (m_x.get()(dim1 - 1) - m_x.get()(dim1 - 2));
    ...
    for (std::size_t i = 1; i < dim1 - 1; ++i) {
        integral += std::exp(1i * std::fmod(omega * m_x.get()(i), 2 * M_PI)) * ((m_deriv2(i + 1, j0) - m_deriv2(i, j0)) / (m_x.get()(i + 1) - m_x.get()(i))
                                                                         - (m_deriv2(i, j0) - m_deriv2(i - 1, j0)) / (m_x.get()(i) - m_x.get()(i - 1)));
    }
    ...
    integral += (eiwtend * m_y.get()(i0, dim1 - 1, im0, im1) - eiwtbegin * m_y.get()(i0, 0, im0, im1)) / (1i * omega) - (eiwtend * deriv1end - eiwtbegin * deriv1begin) / (-omega * omega) + (eiwtend * m_deriv2(dim1 - 1, j0) - eiwtbegin * m_deriv2(0, j0)) / (-1i * omega * omega * omega);
...
void CubicSplineMat<_ScalarT, _n0, _n1, _nm>::fourierTransform(const std::size_t i0, const double omega,
    ...
    std::size_t im0, im1;
    result.resize(m_y.get().dimm(), m_y.get().dimm());  // For blocks, this just does runtime assertion that the new size matches the original size
    for (im1 = 0; im1 < m_y.get().dimm(); ++im1) {
        for (im0 = 0; im0 < m_y.get().dimm(); ++im0) result(im0, im1) = fourierTransform(i0, omega, im0, im1);
    }
}
