...
double longitConduc(const BareHamiltonian& H0, const SqMatArray<std::complex<double>, n0, n1, nm>& selfen, const double beta,
    ...
    assert(selfen.dimm() == H0.numSites());
    ...
    if (H0.type() == "multilayer_in_mag") {
        const Eigen::Index nb_mag = H0.hMultilayerInMag().dim1();
        Eigen::MatrixXcd a(selfen.dimm(), selfen.dimm());
        SqMatArray<std::complex<double>, 1, Eigen::Dynamic, Eigen::Dynamic> A(1, nb_mag, selfen.dimm());  // Purely local
        const auto Hpart = H0.hMultilayerInMag().mastDim0Part();
        
        for (int s = 0; s < 2; ++s) {
            for (iw = 0; iw < selfen.dim1(); ++iw) {
                for (ik = 0; ik < H0.vMultilayerInMag().dim1(); ++ik) {  // dim1() is local size of the number of k-points
                    for (m0 = 0; m0 < nb_mag; ++m0) {
                        a.noalias() = ((engrid(iw) + H0.chemPot()) * Eigen::MatrixXd::Identity(selfen.dimm(), selfen.dimm()) - Hpart(ik, m0) - selfen(s, iw)).inverse();
                        A[m0] = (a - a.adjoint()) / (2i * M_PI);
                        integrand(iw) -= A[m0].cwiseAbs2().sum() * std::norm(H0.vMultilayerInMag()(0, ik, m0, m0));
                    }
                    for (m1 = 0; m1 < nb_mag - 1; ++m1) {
                        for (m0 = m1 + 1; m0 < nb_mag; ++m0)
                            integrand(iw) -= 2.0 * std::real(A[m0].cwiseProduct(A[m1].conjugate()).sum()) * std::norm(H0.vMultilayerInMag()(0, ik, m0, m1));
                    }
                }
            }
        }
        ...
        if (intalg == Simpson) sigmaxx = simpsonIntegrate(integrand, std::real(engrid(1)) - std::real(engrid(0))) / H0.hMultilayerInMag().size() * 2.0 * M_PI * M_PI;
        else if (intalg == CubicSpline) sigmaxx = intvec.dot(integrand.matrix()) / H0.hMultilayerInMag().size() * 2.0 * M_PI * M_PI;
        // Use trapezoidal integration
        else sigmaxx = ((integrand(Eigen::seq(1, Eigen::last)) + integrand(Eigen::seq(0, Eigen::last - 1))) / 2.0
            * (engrid(Eigen::seq(1, Eigen::last)).real() - engrid(Eigen::seq(0, Eigen::last - 1)).real())).sum() / H0.hMultilayerInMag().size() * 2.0 * M_PI * M_PI;
        MPI_Allreduce(MPI_IN_PLACE, &sigmaxx, 1, MPI_DOUBLE, MPI_SUM, selfen.mpiComm());
    }
    else throw std::range_error("Have not implemented calculation of longitudinal conductivity for H0.type() not being multilayer_in_mag");
...
double hallConduc(const BareHamiltonian& H0, const SqMatArray<std::complex<double>, n0, n1, nm>& selfen, const double beta,
    ...
    if (H0.type() == "multilayer_in_mag") {
        const Eigen::Index nb_mag = H0.hMultilayerInMag().dim1();
        Eigen::MatrixXcd a(selfen.dimm(), selfen.dimm());
        SqMatArrayXXXcd A(selfen.dim1(), nb_mag, selfen.dimm());  // Purely local
        const auto Hpart = H0.hMultilayerInMag().mastDim0Part();
        ...
        for (int s = 0; s < 2; ++s) {
            for (ik = 0; ik < H0.vMultilayerInMag().dim1(); ++ik) {  // dim1() is local size of the number of k-points
                for (iw0 = 0; iw0 < selfen.dim1(); ++iw0) {
                    for (m0 = 0; m0 < nb_mag; ++m0) {
                        a.noalias() = ((engrid(iw0) + H0.chemPot()) * Eigen::MatrixXd::Identity(selfen.dimm(), selfen.dimm()) - Hpart(ik, m0) - selfen(s, iw0)).inverse();
                ...
                for (iw1 = 0; iw1 < selfen.dim1(); ++iw1) {
                    ...
                    for (iw0 = 0; iw0 < selfen.dim1(); ++iw0) {
                        ...
                        for (m1 = 0; m1 < nb_mag; ++m1) {
                            for (m0 = 0; m0 < nb_mag; ++m0) {
                                integrand0(iw0) += std::imag(A(iw0, m0).cwiseProduct(A(iw1, m1).conjugate()).sum() * H0.vMultilayerInMag()(0, ik, m0, m1)
                                                             * H0.vMultilayerInMag()(1, ik, m1, m0)) * fermifac;
                            }
                        }
                    }
        ...
        // In units of e^2 / (2 * pi * hbar)
        if (intalg == Simpson) sigmaxy = simpsonIntegrate(integrand1, std::real(engrid(1)) - std::real(engrid(0))) / H0.hMultilayerInMag().size() * 2.0 * M_PI;
        else if (intalg == CubicSpline) sigmaxy = intvec.dot(integrand1.matrix()) / H0.hMultilayerInMag().size() * 2.0 * M_PI;
        // Use trapezoidal integration
        else sigmaxy = ((integrand1(Eigen::seq(1, Eigen::last)) + integrand1(Eigen::seq(0, Eigen::last - 1))) / 2.0
            * (engrid(Eigen::seq(1, Eigen::last)).real() - engrid(Eigen::seq(0, Eigen::last - 1)).real())).sum() / H0.hMultilayerInMag().size() * 2.0 * M_PI;
...
double hallConducCoeff(const BareHamiltonian& H0, const SqMatArray<std::complex<double>, n0, n1, nm>& selfen, const double beta,
    ...
    if (H0.type() == "multilayer_in_mag") {
        if(H0.hMultilayerInMag().dim1() > 1) throw std::runtime_error("hallConducCoeff: Magnetic field is not set to zero");
        Eigen::MatrixXcd A(selfen.dimm(), selfen.dimm());
        Eigen::VectorXd k;
        ...
        for (int s = 0; s < 2; ++s) {
            for (Eigen::Index iw = 0; iw < selfen.dim1(); ++iw) {
                for (Eigen::Index kidlocal = 0; kidlocal < H0.vMultilayerInMag().dim1(); ++kidlocal) {  // dim1() is local size of the number of k-points
                    A.noalias() = ((engrid(iw) + H0.chemPot()) * Eigen::MatrixXd::Identity(selfen.dimm(), selfen.dimm()) - Hpart(kidlocal, 0) - selfen(s, iw)).inverse();
                    A = (A - A.adjoint().eval()) / (2i * M_PI);
                    H0.kVecAtFlatId(kidlocal + Hpart.displ(), k);
                    H0.constructBandCurvature(1, 1, k, epsyy);
                    integrand(iw) += H0.vMultilayerInMag()(0, kidlocal, 0, 0).real() * H0.vMultilayerInMag()(0, kidlocal, 0, 0).real() * epsyy(0, 0).real()
                                     * (A * A * A).trace().real();
                }
            }
        }
        ...
        integrand *= -beta * ebws / (1.0 + ebws).square();
        // In units of e^2 / (2 * pi * hbar)
        if (intalg == Simpson) sigmaHxy = simpsonIntegrate(integrand, std::real(engrid(1)) - std::real(engrid(0))) / H0.hMultilayerInMag().size()
            * (-8.0 * M_PI * M_PI * M_PI * M_PI / 3.0);
        else if (intalg == CubicSpline) sigmaHxy = intvec.dot(integrand.matrix()) / H0.hMultilayerInMag().size() * (-8.0 * M_PI * M_PI * M_PI * M_PI / 3.0);
        // Use trapezoidal integration
        else sigmaHxy = ((integrand(Eigen::seq(1, Eigen::last)) + integrand(Eigen::seq(0, Eigen::last - 1))) / 2.0
            * (engrid(Eigen::seq(1, Eigen::last)).real() - engrid(Eigen::seq(0, Eigen::last - 1)).real())).sum() / H0.hMultilayerInMag().size()
            * (-8.0 * M_PI * M_PI * M_PI * M_PI / 3.0);
