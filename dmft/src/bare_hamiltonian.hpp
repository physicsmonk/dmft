...
private:
    ...
    // Block diagonalized Hamiltonian for the special case, 2D multilayer Hubbard model in magnetic fields. First index runs over k-vectors (ky major)
    // and the second index runs over the eigenvalue space of the block diagonalization.
    SqMatArrayXXXcd m_hMLinMag;
    // Fermi velocity matrices in the space of m_HMag2d for the special case, 2D multilayer Hubbard model in magnetic fields.
    // Local-size: first index is just x or y, second index runs over local k-vectors (ky major)
    SqMatArray2XXcd m_vMLinMag;
    std::string m_type;
...
protected:
    Eigen::Index m_nsites;  // Number of sites in unit cell without magnetic field
...
public:
    ...
    Eigen::Index numSites() const {return m_nsites;}
    void numSites(const Eigen::Index nsites) {m_nsites = nsites;}
    ...
    const SqMatArrayXXXcd& hMultilayerInMag() const {return m_hMLinMag;}
    const SqMatArray2XXcd& vMultilayerInMag() const {return m_vMLinMag;}
    ...
    SqMatArray22Xcd& moments() {return m_moments;}
};
...
void BareHamiltonian::computeBands(const Eigen::DenseBase<Derived>& nk, const Eigen::DenseBase<OtherDerived>& kidpath) {
    ...
    if (m_type == "multilayer_in_mag") {
        if (m_a.cols() != 2) throw std::invalid_argument("Space dimension must be 2 for 2D multilayer Hubbard model in magnetic fields!");
        if (nbands % m_nsites != 0) throw std::range_error("Hamiltonian's dimension must be multiple of number of sites for 2D multilayer Hubbard model in magnetic fields!");
        const Eigen::Index nb_mag = nbands / m_nsites;
        Eigen::SelfAdjointEigenSolver<Eigen::MatrixXcd> es(nb_mag);
        Eigen::MatrixXcd fv;
        m_hMLinMag.mpiComm(m_comm);
        m_hMLinMag.resize(nkt, nb_mag, m_nsites);  // Allocate resources for _HdimerMag2d, full-size data is needed by all processes
        auto Hpart = m_hMLinMag.mastDim0Part();  // Choose only partitioning the first dimension
        m_vMLinMag.resize(2, Hpart.dim0(), nb_mag);   // No need to gather, so just allocate local-size data
        for (Eigen::Index kidlocal = 0; kidlocal < Hpart.dim0(); ++kidlocal) {
            kVecAtFlatId(kidlocal + Hpart.displ(), k);
            constructHamiltonian(k, H);  // Call the implemented method in derived classes
            ...
            es.compute(H.topLeftCorner(nb_mag, nb_mag));  // Only the lower triangular part is used
            // Assemble the block Hamiltonian in the dimer space
            for (Eigen::Index m = 0; m < nb_mag; ++m) {
                //Hmastpart(kidlocal, m) << es.eigenvalues()(m), std::conj(H(nb_2, 0)),
                //H(nb_2, 0),          es.eigenvalues()(m);
                Hpart(kidlocal, m).diagonal().setConstant(es.eigenvalues()(m));
                for (Eigen::Index l1 = 0; l1 < m_nsites - 1; ++l1) {
                    for (Eigen::Index l0 = l1 + 1; l0 < m_nsites; ++l0) {
                        Hpart(kidlocal, m, l0, l1) = H(l0 * nb_mag, l1 * nb_mag);
                        Hpart(kidlocal, m, l1, l0) = std::conj(Hpart(kidlocal, m, l0, l1));
                    }
                }
            }
            ...
            for (int co = 0; co < 2; ++co) {
                ...
                m_vMLinMag(co, kidlocal).noalias() = es.eigenvectors().adjoint() * fv.topLeftCorner(nb_mag, nb_mag).selfadjointView<Eigen::Lower>() * es.eigenvectors();
            }
        }
        Hpart.allGather();   // All gather because all processes need the full data
    }
}
...
void computeLattGFfCoeffs(const BareHamiltonian& H0, const SqMatArray<std::complex<double>, n0, n1, nm>& selfen_dyn,
    ...
    const Eigen::Index nsites = selfen_dyn.dimm();
    assert(nsites == H0.numSites());
    Gw.mpiComm(selfen_dyn.mpiComm());
    Gw.resize(selfen_dyn.dim0(), selfen_dyn.dim1(), nsites);
    const auto selfen_dyn_part = selfen_dyn.mastFlatPart();
    auto Gwpart = Gw.mastFlatPart();
    std::array<Eigen::Index, 2> so;
    std::complex<double> wu;
    
    /*
    if (nsites == 1) {  // Single site case, where we can utilize the noninteracting density of states
     ...
     */
    if (H0.kPrimVecs().rows() == 0) throw std::range_error("Reciprocal primative vectors have not been set!");
    ...
    if (H0.type() == "multilayer_in_mag") {
        if (H0.hMultilayerInMag().size() == 0) throw std::range_error("Block Hamiltonian of the 2D dimer Hubbard model in magnetic fields has not been computed!");
        Eigen::Index ist;
        Gwpart().setZero();
        for (Eigen::Index i = 0; i < selfen_dyn_part.size(); ++i) {
            so = selfen_dyn_part.global2dIndex(i);  // Get the index in (spin, omega) space w.r.t. the full-sized data
            //    if constexpr (std::is_same<typename Derived::Scalar, std::complex<mpfr::mpreal> >::value) {
            //        wu.real(energies(so[1]).real().toDouble() + H0.chemPot());
            //        wu.imag(energies(so[1]).imag().toDouble());
            //    }
            //    else wu = static_cast<std::complex<double> >(energies(so[1])) + H0.chemPot();
            wu = energies(so[1]) + H0.chemPot();
            for (ist = 0; ist < H0.hMultilayerInMag().size(); ++ist) Gwpart[i] += -(wu * Eigen::MatrixXd::Identity(nsites, nsites) - H0.hMultilayerInMag()[ist]
                                                                            - selfen_dyn_part[i] - selfen_static[so[0]]).inverse();
            Gwpart[i] /= static_cast<double>(H0.hMultilayerInMag().size());
        }
    }
    else {
        ...
        for (Eigen::Index i = 0; i < selfen_dyn_part.size(); ++i) {
            ...
            for (ik = 0; ik < nk; ++ik) {
                ...
                Gwpart[i] += -(wu * Eigen::MatrixXcd::Identity(nsites, nsites) - H.selfadjointView<Eigen::Lower>() * Eigen::MatrixXcd::Identity(nsites, nsites)
                               - selfen_dyn_part[i] - selfen_static[so[0]]).inverse();
            }
            Gwpart[i] /= static_cast<double>(nk);
        }
    }
    //}
}
...
void computeSpectraKW0(const BareHamiltonian& H0, const SqMatArray<std::complex<double>, n0, n1, nm>& selfen, const Eigen::Index id0,
    ...
    if (H0.type() == "multilayer_in_mag") {
        for (Eigen::Index i = 0; i < A0part.size(); ++i) {
            ...
            A0part[i].setZero();
            for (Eigen::Index m = 0; m < H0.hMultilayerInMag().dim1(); ++m)
                A0part[i] += -(H0.chemPot() * Eigen::MatrixXd::Identity(selfen.dimm(), selfen.dimm()) - H0.hMultilayerInMag()(sk[1], m) - selfen(sk[0], id0)).inverse();
            A0part[i] = (A0part[i] - A0part[i].adjoint().eval()) / (static_cast<double>(H0.hMultilayerInMag().dim1()) * 2i * M_PI);
        }
    }
    else {
        Eigen::VectorXd k;
...
void computeSpectraKW(const BareHamiltonian& H0, const SqMatArray<std::complex<double>, n0, n1, nm>& selfen, const Eigen::DenseBase<Derived>& energies,
    ...
    if (H0.type() == "multilayer_in_mag") {
        ...
        for (Eigen::Index i = 0; i < Akwpart.size(); ++i) {
            ...
            Akwpart[i].setZero();
            for (Eigen::Index m = 0; m < H0.hMultilayerInMag().dim1(); ++m)
                Akwpart[i] += -((energies(iw) + H0.chemPot()) * Eigen::MatrixXd::Identity(selfen.dimm(), selfen.dimm()) - H0.hMultilayerInMag()(kid, m)
                                - selfen(swk[0], iw)).inverse();
            Akwpart[i] = (Akwpart[i] - Akwpart[i].adjoint().eval()) / (static_cast<double>(H0.hMultilayerInMag().dim1()) * 2i * M_PI);
        }
    }
    else {
        Eigen::VectorXd k;
